...
void HardDrone::setOrientation(float yaw, float pitch, float roll)
{
...
void HardDrone::move(float dt)
{
    ...
    // rotate local force to world using quaternion (correct order)
    Vector3f propellerForceWorld = orientationQ * propellerForceLocal;

    Vector3f af{0.0f, 0.0f, -9.81f * m};
    Vector3f totalForce = propellerForceWorld + af ;
    acc = totalForce / m;
    vel += acc * dt;
    position += vel * dt;
    ...
    Vector3f totalMomentLocal = yxMomentLocal + _zMomentLocal;

    Vector3f totalMomentWorld = orientationQ * totalMomentLocal;
    
    angularAcc = totalMomentWorld / angularInertia;
